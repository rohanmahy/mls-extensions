...
interact with MLS to take advantage of security features of MLS.
...
As application designers become familiar with MLS, there is interest in
leveraging other cryptographic tools that an MLS group provides:
...
MLS specification {{!RFC9420}}. In addition, we introduce the following new
terms:
...
*components*). Furthermore, it is essential that application components do not
...
the appropriate calculations in the protocol to achieve the required separation.
...
pair for any operation defined in {{!RFC9180}}, such as encryption, exporting
keys, and the PSK mode, as long as the `info` input to `Setup<MODE>S` and
`Setup<MODE>R` is set to ComponentOperationLabel with `component_id` set to the
appropriate ComponentID. The `context` can be set to an arbitrary Context
...
- The key pair of a non-blank ratchet tree node. The PrivateKey of such a key
  ...
  PublicKey does not change. The key pair of the root node SHOULD NOT be used,
...
the client assembles a list of AppDataUpdate proposals with that `component_id`,
...
* If the list comprises one or more proposals, all with `op` field set to
  `update`:
    ...
    * The application logic returns either an opaque value `new_data` that will
    ...
    * If no `app_data_dictionary` extension is present in the GroupContext, add
      one to the end of the `extensions` list in the GroupContext.
...
AppEphemeral proposals do not require an UpdatePath. An AppEphemeral proposal
...
Likewise, the `required_capabilities` GroupContext extension (defined in
{{Section 11.1 of !RFC9420}} and reproduced below) contains all mandatory to
support non-default extensions in its `extension_types` vector. Its
`proposal_types` vector contains any mandatory to support Proposals. Its
`credential_types` vector contains any mandatory credential types.
...
`supported_wire_formats` (which can appear in LeafNodes), and
...
This document also defines new components of the `app_data_dictionary` extension
for supported and required Safe AAD, media types, and components.
...
Finally, the supported and required media types (formerly called MIME types) are
communicated in the `content_media_types` component (see
...
An AppAck object is used to acknowledge receipt of application messages. Though
this information implies no change to the group, it is conveyed inside an
AppEphermeral Proposal with a component ID `app_ack`, so that it is included in
the group's transcript by being included in Commit messages.
...
An AppAck represents a set of messages received by the sender in the current
epoch. Messages are represented by the `sender` and `generation` values in the
MLSCiphertext for the message. Each MessageRange represents receipt of a span of
messages whose `generation` values form a continuous range from
`first_generation` to `last_generation`, inclusive.

AppAck objects are sent as a guard against the Delivery Service dropping
application messages. The sequential nature of the `generation` field provides a
degree of loss detection, since gaps in the `generation` sequence indicate
...
This section defines a minimal framing format so MLS clients can signal which
media type is being sent inside the MLS `application_data` object when multiple
formats are permitted in the same group.

It also defines a new `content_media_types` application component which is used
...
`application_data` will be framed using the application framing format described
later in {{app-framing}}. This allows clients to confirm that all members of a
...
type and subtype) and any of its parameters. Even if the `parameter_value` would
have required formatting as a `quoted-string` in a text encoding, only the
contents inside the `quoted-string` are included in `parameter_value`. Likewise,
only the second character of a `quoted-pair` is included in `parameter_value`;
...
`app_data_dictionary` extension) in the GroupContext. (The creating client also
includes its `content_media_types` component in its own LeafNode as described in
...
not supported by all members in the resulting epoch.
...
The design of the MLS protocol prevents a member of an MLS group from removing
...
and wait for another member to Commit its Proposal.

Unfortunately, MLS clients that join via an External Commit ignore pending, but
...
SelfRemove cannot be sent as an external proposal.
...
Whenever a member receives a SelfRemove Proposal, it includes it along with any
other pending Propsals when sending a Commit. It already MUST send a Commit of
pending Proposals before sending new application messages.

When a member receives a Commit referencing one or more SelfRemove Proposals, it
treats the proposal like a Remove Proposal, except the leaf node to remove is
determined by looking in the Sender `leaf_index` of the original Proposal. The
member is able to verify that the Sender was a member.
...
SelfRemove proposals). The new joiner MUST validate the SelfRemove Proposal
...
During validation, SelfRemove proposals are processed after Update proposals and
...
Remove proposal is invalid. A client MUST NOT issue more than one SelfRemove
proposal per epoch.

The MLS Delivery Service (DS) needs to validate SelfRemove Proposals it receives
(except that it cannot validate the `membership_tag`). If the DS provides a
GroupInfo object to an external joiner, the DS SHOULD attach any SelfRemove
proposals known to the DS to the GroupInfo object.

As with Remove proposals, clients need to be able to receive a Commit message
which removes them from the group via a SelfRemove. If the DS does not forward a
Commit to a removed client, it needs to inform the removed client out-of-band.
...
other clients haven't yet been upgraded.

New credential types `MultiCredential` and `WeakMultiCredential` are defined as
shown below. These credential types are indicated with the values `multi` and
`weak-multi` (see {{iana-creds}}).
...
A client that receives a credential of type `weak-multi` in a LeafNode MUST
...
This document requests the addition of various new values under the heading of
...
RFC EDITOR: Please replace XXXX throughout with the RFC number assigned to this
document
...
The `supported_wire_formats` MLS Extension Type is used inside LeafNode objects.
It contains a list of non-default Wire Formats supported by the client node.
...
The `self_remove` MLS Proposal Type is used for a member to remove itself from a
group more efficiently than using a `remove` proposal type, as the `self_remove`
type is permitted in External Commits.
...
application. MLS implementations MUST NOT impose restrictions on where component
...
`generation` sequence indicate dropped messages. When AppAck messages are
...
- The application might have the committer include an AppAck whenever a Commit
...
An application using AppAck to guard against loss/suppression of application
...
`membership_tag`. However, an external joiner also has no way to completely
...
addition of this proposal type.
